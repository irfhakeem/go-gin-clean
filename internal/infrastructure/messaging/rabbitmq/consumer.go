...
func NewConsumer(conn *amqp.Connection, exchange string, handlers map[string]EventHandlerFunc) port.Consumer {
	return &consumer{
		conn:     conn,
		exchange: exchange,
		handlers: handlers,
...
func (c *consumer) handleDelivery(ctx context.Context, d amqp.Delivery, retryQueue string) {
	...
	var consumerErr *pkgerrors.ConsumerError
	if errors.As(err, &consumerErr) &&
		consumerErr.Type == pkgerrors.Retryable &&
